...
def filter_pr(pr):
    def apply(x):
        return {"pr": testmerge for testmerge in x['data'].values() if str(testmerge['number']) == str(pr)}
    return apply

def get_tm_rounds(engine, pr):
    with Session(engine) as session:
        query = session.query(Feedback.round_id, Feedback.json, Feedback.datetime).where(and_(
                Feedback.key_name == 'testmerged_prs',
                Feedback.json["data"].regexp_match(f'"number": "{pr}"'))).order_by(Feedback.round_id)
...
def get(pr: str, connection_string: str):
    ...
    tm_rounds = tm_rounds.join(pd.json_normalize(tm_rounds.json.map(filter_pr(pr)))).drop(['json'], axis=1)
